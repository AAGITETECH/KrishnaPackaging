...
namespace KrishnaPackaging.View
{
    public partial class InwardAdd : Form
    {
        ...
        void FillItemDetail()
        {
            ReceiveNoteDetail _Rm = _KPDB.ReceiveNoteDetails.Where(w => w.IMId == Convert.ToInt64(CmbItem.SelectedValue) && w.IsDelete != true).SingleOrDefault();
            if (_Rm != null)
            {
                decimal Qty = Convert.ToDecimal(_Rm.Qty) - Convert.ToDecimal(_Rm.InwardQty);
                if (Qty != 0)
                {
                    txtTotalQty.Text = Qty.ToString("N2");
                    ...
                    {
                        txtQty.Text = Convert.ToDecimal(_Rm.Qty).ToString("N2");
                    }
                }
            }
        }
